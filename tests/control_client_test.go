...
func TestControlPlaneClient(t *testing.T) {
	...
	t.Run("FetchSnapshot Success", func(t *testing.T) {
		expectedSnapshot := runtime.TenantSnapshot{
			ProjectID: projectID,
			Routes: []model.Route{
			...
			APIKeys: []runtime.TenantAPIKeyDTO{
		...
		client := runtime.NewControlPlaneClient(server.URL, projectID.String(), token, logger)
	...
	t.Run("FetchSnapshot Unreachable Server", func(t *testing.T) {
		client := runtime.NewControlPlaneClient("http://localhost:9999", projectID.String(), token, logger)
...
func TestWarmAPIKeyCache(t *testing.T) {
	...
	loader := runtime.NewLoader(nil, rdb, logger, 10*time.Second)

	t.Run("Warm Cache with Valid Keys", func(t *testing.T) {
		keys := []runtime.TenantAPIKeyDTO{
